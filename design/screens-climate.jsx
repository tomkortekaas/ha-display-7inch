...
function ClimateScreen({ data, setData, accents }) {
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 14, height: "100%" }}>
      ...
      <div className="card" style={{ display: "flex", flexDirection: "column", padding: 18 }}>
        ...
        <div style={{ flex: 1, display: "grid", placeItems: "center", position: "relative" }}>
          <Ring size={260} stroke={20} value={setpoint - 10} max={18} color={accents.heat} dim={`${accents.heat}22`}>
            <div style={{ fontSize: 11, color: "var(--ink-dim)", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>ingesteld</div>
            <div className="ring-value" style={{ fontSize: 86, marginTop: 4 }}>{setpoint.toFixed(1)}<span style={{ fontSize: 32, color: "var(--ink-dim)" }}>°</span></div>
